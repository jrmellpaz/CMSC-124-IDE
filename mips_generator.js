    ...
    handleNode(node) {
        ...
        switch (node.type) {
            ...
            case 'output':
                ...
                // Define the variableRegex to match variables in curly braces
                const variableRegex = /\{(.*?)\}/;

                // Initialize message with the node message
                let message = node.message;

                // Match the variable in the output message (e.g., {z})
                ...
                if (variable) {
                    ...
                    if (variableValue !== null) {
                        ...
                        this.textSection.addInstruction(`li $a0, ${variableValue}`);  // Load the value of the variable into $a0
                        this.textSection.addInstruction(`li $v0, 1`);                  // Set the syscall for printing an integer (1)
                        this.textSection.addInstruction(`syscall`);                    // Execute the syscall to print the value
                    } else {
                    ...
                    this.textSection.addInstruction(`li $v0, 4`);  // Set the syscall for printing a string (4)
                    this.textSection.addInstruction(`la $a0, message`);  // Load the address of the string message into $a0
                    this.textSection.addInstruction(`syscall`);  // Execute the syscall to print the string
                }
